...
def load_model(expr_dir, model_code=None, remove_words_in_model_weights=None, load_only_ps=False, map_location = "cuda"):
    '''
    ...
    state_dict = torch.load(trained_weigths_fname,map_location=map_location)['state_dict']
...
if __name__ == '__main__':
    ...
    sc_work_basedir = '/ps/scratch/human_body_prior/VPoser/training_experiments'

    sc_expr_id = 'V02_05'

    sc_expr_dir = osp.join(sc_work_basedir, sc_expr_id)

    model, model_ps = load_model(sc_expr_dir, model_code=VPoser, remove_words_in_model_weights='vp_model.')
    print(model)
